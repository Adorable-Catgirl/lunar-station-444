...
local function get_int(size, int)
	local str = ""
	for i=1, size do
		str = str .. string.char(band(int, 0xFF))
		int = rshift(int, 8)
	end
	return str
end
...
local function write_tag(type, flags, data)
	local dsize = #data
	if (dsize <= 15) then
		return string.char(type, bor(lshift(bor(flags, tag_flags.internal), 4), dsize))..data
	end
	local isize = get_int_size(dsize)
	return string.char(type, bor(lshift(flags, 4), isize))..get_int(isize, dsize)..data
end
...
return {serialize=serialize_table, deserialize=deserialize}